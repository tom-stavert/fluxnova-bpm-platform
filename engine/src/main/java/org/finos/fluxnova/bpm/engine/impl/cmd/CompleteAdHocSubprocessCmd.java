 ...
 *
 * Respects {@code cancelRemainingInstances}: if {@code true} (the default),
 * active children are cancelled before leaving; if {@code false}, the
 * subprocess transitions into a draining state (no new triggers allowed) and
 * leaves once all active children have finished naturally.
 */
public class CompleteAdHocSubprocessCmd implements Command<Void>, Serializable {
  ...
  public Void execute(CommandContext commandContext) {
    ...
    ((AdHocSubProcessActivityBehavior) behavior).complete(execution);
    return null;
  }
